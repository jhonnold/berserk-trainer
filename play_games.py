...
def run_match(best, root_dir, c_chess_exe, concurrency, book_file_name, berserk_base, berserk_test):
...
def run_round(
    ...
    if len(networks) == 0:
        print("No berserk-*.nn files found in {}".format(root_dir))
        time.sleep(10)
        return
    else:
        print("Found {} berserk-*.nn files".format(len(networks)))
    ...
    run_match(best, root_dir, c_chess_exe, concurrency, book_file_name, berserk_base, berserk_test)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Finds the strongest .nn in tree, playing games.",
    ...
    parser.add_argument(
        ...
        type=str,
        default="./berserk",
        help="Path to berserk master (reference version), see https://github.com/jhonnold/berserk",
    )
    parser.add_argument(
        "--berserk_test",
        type=str,
        help="(optional) Path to new berserk binary, if not set, will use berserk_base",
    )
    parser.add_argument(
        "--book_file_name",
        type=str,
